...
Function EsOpArit(var Fuente:t_arch;var Control:Longint;var CompLex:TSimbgramatical;var Lexema:string): Boolean;
...
End;
  ...
  begin
    ...
    //Recorrer la cadena
    EstadoActual:=q0;
    while not (EstadoActual in [2,1]) do
      begin
        leercaracter(Fuente,nuevo_control,caracter);
        EstadoActual:=Delta[EstadoActual,CarASimb(caracter)];
        if EstadoActual=1 then
...
begin
    ...
    If carac=finarch then CompLex:=pesos
     ...
     begin
      ...
      else if EsOpArit(Fuente,Control,CompLex,Lexema) then
                  begin
                  //CompLex:=TOpArit;
                  //CompLex01:='OpArit';
                  //InstalarEnTS(Lexema,TS,CompLex)
